...
def escrever_subgrafos(subgrafos, lista_de_adjacencia):
    with open("Subgrafos.txt", "w") as f_subgrafos:
        for i, (vertice_de_corte, subgrafo, componentes) in enumerate(subgrafos):
            f_subgrafos.write(f"Subgrafo {i + 1} (Removido o vértice de corte {vertice_de_corte}):\n")
...
def plotar_grafo_com_vertices_de_corte(grafo, vertices_de_corte):
    pos = nx.circular_layout(grafo)  # Use o layout circular
...
def main():
    ...
    # Ajuste o valor de k para controlar a distância entre os vértices
    pos = nx.spring_layout(grafo, k=0.2)  # Experimente diferentes valores de k
    ...
    with open("VerticesDeCorte.txt", "w") as f_vertices_de_corte:
        for vertex in vertices_de_corte:
            f_vertices_de_corte.write(f"Vértice de corte: {vertex}\n")

    subgrafos = encontrar_subgrafos_apos_remocao(vertices_de_corte, lista_de_adjacencia)

    for _, subgrafo, _ in subgrafos:
        vertices_encontrados.update(subgrafo.nodes())

    porcentagem_encontrada = len(vertices_encontrados) / total_vertices_originais * 100
